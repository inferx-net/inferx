...
- Merged into `merged.md` (full), `llm.md` (LLM-optimized), `index.md` (knowledge index), `merged_compressed.md` (lossless)
...
```bash
sudo docker run --rm \
  -v /home/brad/rust/test/input:/input \
  -v /home/brad/rust/test/output:/output \
...
- `/home/brad/test/output/index.md`: Knowledge base index generated by LLM (topics, entities, search hints)
...
### Knowledge Index Output Format (`index.md`)

Complete LLM prompt file with system instruction, knowledge index, and full documents:
...
Feed this file directly to an LLM as a complete prompt.
...
### Knowledge Index Features:
- LLM-generated domain overview
- Key topics and concepts from all documents
- Document summaries with main points
- Key entities and searchable terms
- Quick search hints for navigation
