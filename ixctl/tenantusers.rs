...
impl TenantUsersCmd {
    ...
    pub fn SubCommand<'a, 'b>() -> App<'a, 'b> {
        return SubCommand::with_name("tenantusers")
            ...
            .about("get users with tenant role");
    }

    pub async fn Run(&self, gConfig: &GlobalConfig) -> Result<()> {
        println!("TenantUsersCmd is {:#?}", self);
        ...
        let users = match client
            .TenantUsers(&gConfig.accessToken, &role, &self.tenant)
            .await
        {
            Err(e) => {
                println!("doesn't find TenantUsersCmd with {:#?}", e);
                return Ok(());
            }
            Ok(users) => users,
        };
